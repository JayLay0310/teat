...
public class CryptoVerificationController {
    ...
    public Result verifyFile(
            @ApiParam(value = "操作类型：HASH/ENCRYPT/DECRYPT/SIGN/VERIFY", required = true)
            @RequestParam(value = "operation", required = true) String operation,

            @ApiParam(value = "算法名称", required = true)
            @RequestParam(value = "algorithm", required = true) String algorithm,

            @ApiParam(value = "要处理的文件", required = true)
            @RequestParam(value = "file", required = true) MultipartFile file,

            @ApiParam(value = "文件格式：BINARY/HEX/BASE64", defaultValue = "BINARY")
            @RequestParam(value = "fileFormat", required = false, defaultValue = "BINARY") String fileFormat,

            @ApiParam(value = "其他参数(JSON格式)")
            @RequestParam(value = "params", required = false) String paramsJson) {

        logger.info("接收到文件{}操作请求，算法：{}", operation, algorithm);
        ...
        try {
            ...
            if (paramsJson != null && !paramsJson.isEmpty()) {
                try {
                    // 使用Spring自带的JSON解析器，不依赖Jackson的TypeReference
                    params = new org.springframework.boot.json.JacksonJsonParser().parseMap(paramsJson);
                } catch (Exception e) {
            ...
            // 对于哈希操作，确保参数中包含盐值和迭代次数信息
            if ("HASH".equalsIgnoreCase(operation)) {
            ...
            Map<String, Object> result = cryptoService.verifyFile(
                    operation, algorithm, file, fileFormat, params);
